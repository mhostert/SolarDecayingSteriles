...
from source import flavour_transitions as std_osc
...
ax.contour(X,Y,LK, [chi2.ppf(0.90, dofK)], colors=['blue'],linewidths=[1.0],label=r'KamLAND')
# ax.contour(X,Y,L, 20, color='black')

# ax.contourf(X,Y,LB, [chi2.ppf(0.90, dofB),1e100], colors=['black'],alpha=0.1, linewidths=[0.1])
ax.contour(X,Y,LB, [chi2.ppf(0.90, dofB)], colors=['red'],linewidths=[1.0],label=r'Borexino')
Z = LB
# pcm = ax.pcolor(X, Y, Z,
...
ax.contour(X,Y,LS, [chi2.ppf(0.90, dofS)], colors=['green'],linewidths=[1.0], ls='--',label=r'SuperK-IV')
...
ax.plot([],[],color='blue', lw =1,label=r'KamLAND')
ax.plot([],[],color='red', lw =1,label=r'Borexino')
ax.plot([],[],color='green', lw =1,label=r'SuperK-IV')
